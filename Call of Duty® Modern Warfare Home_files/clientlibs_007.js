...
(function($, ATVI) {
    ...
    var init = function() {
        if (dataLayer && digitalData) ATVI.definePageView();
        else {
            document.addEventListener('gtm_loaded', function() {
                if (digitalData) ATVI.definePageView();
            });
        }
    };
    ...
    ATVI.definePageView = function() {
        var url = window.location.href;
        if (url.indexOf('activisionblizzard') > -1 || url.indexOf('my.callofduty') > -1 || url.indexOf('stage.callofduty.com') > -1 || url.indexOf('blog.activision.com') > -1) return;
...
// cookies banner pop up
(function() {

    var initCookies = function() {

        if(typeof ATVI.utils.getCookie("atvi-cookie") == "undefined") {

            var banner = $(".atvi-cookie-disclaimer");
            if(banner.length) {
                banner.addClass("show");
            }
    
            var btn = banner.find("button, .cookies-close, .close-cookies");
            btn.click(function(e){
                e.preventDefault();
                banner.removeClass("show");
                setCookie('atvi-cookie');
            }); 
        }

        if (typeof ATVI.utils.getCookie('atvi-privacy-policy-updated') == 'undefined') {
			var $el = $('.atvi-privacy-policy-module');
            if ($el.length) {
                $el.addClass('show');
                $el.find('.close-modal').on('click', function() {
					$el.removeClass('show');
                    setCookie('atvi-privacy-policy-updated');
                });
            }
        }
    };
